...
export async function StudentDashboard({ userId }: { userId: string }) {
    const requestHistory = await db.borrowRequest.findMany({
        where: {
            studentId: userId,
        },
        include: {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center mb-4">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-800">My Borrowing History</h2>
            ...
            {requestHistory.length === 0 ? (
                <div className="w-full bg-white border border-gray-100 shadow-sm rounded-xl text-center py-12 flex flex-col items-center justify-center space-y-3">
                    ...
                    <h3 className="text-lg font-semibold text-gray-700">No Request History Yet</h3>
                    <p className="max-w-sm mx-auto text-gray-500 text-sm px-4 sm:px-0">
                        Your submitted requests and borrow history will appear here. Click the button above to submit your first request.
                    </p>
                </div>
            ) : (
                <StudentActiveRequestsList initialRequests={requestHistory} />
            )}
        </div>
    )
}
